...
//
// Usage: main_serve_bench [n_requests=64] [gemm=mine|cublas] [slots=1,2,4,8,16,32]
//                         [max_new_base=48] [max_seq=192]
...
struct Row {
    ...
    req_p50, req_p95, ttft_p50; int64_t steps, tokens;
};
...
int main(int argc, char** argv) {
    ...
    const int n_requests = argc > 1 ? std::atoi(argv[1]) : 64;
    const std::string gemm_s = argc > 2 ? argv[2] : "mine";
    const std::vector<int> slot_list = parse_list(argc > 3 ? argv[3] : "1,2,4,8,16,32");
    const int max_new_base = argc > 4 ? std::atoi(argv[4]) : 48;
    const int64_t max_seq = argc > 5 ? std::atoll(argv[5]) : 192;
    ...
    for (int i = 0; i < n_requests; i++) {
        llm::Request r;
        r.id = i;
        r.prompt = prompts[size_t(i) % prompts.size()];
    ...
    std::printf("model %s gemm=%s: %d requests, %lld tokens to generate, max_seq %lld\n",
                ...
                (long long)max_seq);
    std::printf("%5s %10s %9s %8s %10s %9s %9s %8s %6s\n", "slots", "mode", "tok/s", "step ms",
                "tok lat ms", "req p50", "req p95", "ttft p50", "steps");

    std::vector<Row> rows;
    for (int slots : slot_list) {
        for (bool continuous : {true, false}) {
            llm::GpuBatch batch(gpu, slots, max_seq, gemm);
            ...
            Row row{slots, continuous, wall_ms, tokens / (wall_ms / 1e3), wall_ms / double(sch.steps()),
                    ...
                    sch.steps(), tokens};
            rows.push_back(row);
            std::printf("%5d %10s %9.1f %8.2f %10.2f %9.0f %9.0f %8.0f %6lld\n", slots,
                        continuous ? "continuous" : "static", row.tok_s, row.step_ms, row.token_lat_ms,
                        row.req_p50, row.req_p95, row.ttft_p50, (long long)row.steps);
    ...
    const std::string path = root + "/bench/stage6_serve_" + llm::model_name() + "_" + gemm_s + "_" + ts + ".json";
    std::ofstream f(path);
